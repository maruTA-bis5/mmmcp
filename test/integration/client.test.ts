...
describe("mattermost authentication", async () => {
    it("should throw an error when the access token is invalid", async () => {
        await expect(MattermostClient.create({ url: getMattermostUrl(), auth: { token: 'invalid' } })).rejects.toThrow('Invalid Mattermost personal access token');
    });
    it("should throw an error when the username is invalid", async () => {
        await expect(MattermostClient.create({ url: getMattermostUrl(), auth: { username: 'invalid', password: 'invalid' } }))
    ...
    it("should throw an error when the password is invalid", async () => {
        await expect(MattermostClient.create({ url: getMattermostUrl(), auth: { username: 'general', password: 'invalid' } }))
            .rejects.toSatisfy(e => assertServerErrorId(e, 'api.user.login.invalid_credentials_email_username'));
    });
});
