...
import { GetChannelMembersTool } from '../../../../src/tools/channels/get-channel-members.js';
...
describe('get_channel_members tool', () => {
    it('should return members of a Mattermost channel', async () => {
        ...
        try {
            const members = await client.api.getChannelMembers(channel.id);
            const getChannelMembersTool = new GetChannelMembersTool(client);
            const result: ToolResult = await execute(() =>
                getChannelMembersTool.definition.handler(client, { channel_id: channel.id }),
            );
