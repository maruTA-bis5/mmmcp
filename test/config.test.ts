...
describe('parseConfig', () => {
    it('uses command-line URL over the environment and accepts readonly mode', () => {
        const config = parseConfig(['--url', 'https://cli.example.test/path', '--readonly', '--log-level', 'debug'], {
            MATTERMOST_URL: 'https://environment.example.test',
    ...
    it('preserves a Mattermost reverse-proxy path', () => {
        const config = parseConfig(['--url', 'https://mattermost.example.test/chat/'], { MATTERMOST_TOKEN: TOKEN });
