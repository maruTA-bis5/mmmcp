...
export function parseConfig(
    ...
    for (let index = 0; index < args.length; index += 1) {
        ...
        if (argument === '--url' || argument === '--log-level') {
            ...
            if (argument === '--url') {
                ...
                throw new ConfigurationError(`Invalid log level "${value}". Use debug, info, warn, or error.`);
            }
            continue;
        }
    ...
    if (mattermostUrl === undefined || mattermostUrl.trim() === '') {
        throw new ConfigurationError('Mattermost URL is required. Provide --url or MATTERMOST_URL.');
    }
    ...
    if (!hasToken && (!hasUsername || !hasPassword)) {
        throw new ConfigurationError('Set MATTERMOST_TOKEN or both MATTERMOST_USERNAME and MATTERMOST_PASSWORD.');
    }
    ...
    try {
        ...
        normalizedUrl = `${parsedUrl.origin}${parsedUrl.pathname}`.replace(/\/+$/, '') || parsedUrl.origin;
    } catch {
